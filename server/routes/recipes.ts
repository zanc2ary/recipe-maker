  ...
  try {
    ...
    try {
      ...
      const ingredientBasedRecipes = mockRecipeData.map((recipe, index) => ({
        ...
        ingredients: [...ingredients, ...recipe.ingredients.filter(ing => !ingredients.includes(ing))],
      }));
    ...
    res.status(500).json({ error: "Failed to get recipe recommendations", details: error.message });
  }
};
